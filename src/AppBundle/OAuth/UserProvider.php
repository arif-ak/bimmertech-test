...
class UserProvider extends BaseUserProvider implements AccountConnectorInterface, OAuthAwareUserProviderInterface
{
    ...
    private function createUserByOAuthUserResponse(UserResponseInterface $response): SyliusUserInterface
    {
        ...
        if (!$customer->getPath() || !$user->getPath()) {
            $hash = bin2hex(random_bytes(16));

            $path = $this->expandPath($hash);
            $dir = "media/image/$path";
            ...
            $file = file_get_contents($response->getProfilePicture());
            file_put_contents('media/image/' . $filePath, $file);

            !$user->getPath() ? $user->setPath($filePath) : null;
            !$customer->getPath() ? $customer->setPath($filePath) : null;
        }
